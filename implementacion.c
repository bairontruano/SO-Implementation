...
void read_processes(char *filename, Process processes[], int *n) {
    ...
    *n = 0;
    ...
    while (fscanf(file, "%d %d %d %d", &processes[*n].pid, &processes[*n].arrival_time, 
        ...
        processes[*n].start_time = -1;  //-1 indica que aún no se ha ejecutado
...
void mlfq(Process queue1[], Process queue2[], Process queue3[], int count1, int *count2, int *count3, int quantum[], int *total_waiting_time, int *total_turnaround_time) {
    ...
    while (cola_index < 3) {
        for (int i = 0; i < *current_count; i++) {
            ...
            if (current_queue[i].start_time == -1) {  //Inicializar start_time solo una vez
            ...
            if (current_queue[i].remaining_time > time_slice) {
                ...
                //El proceso termina
                current_time += current_queue[i].remaining_time;
                current_queue[i].remaining_time = 0;
                current_queue[i].completed_time = current_time;
                current_queue[i].waiting_time = current_queue[i].start_time - current_queue[i].arrival_time;
                current_queue[i].turnaround_time = current_queue[i].completed_time - current_queue[i].arrival_time;
